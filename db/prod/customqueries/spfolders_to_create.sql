;with config as (
	...
	select project_id, id, concat(id, '-', case when task_type = '' then 'translate' else task_type end) as taskn , case when task_type = '' then 'translate' else task_type end as task_type from tasks
), pr as (
