...
func NewAuthGraphClient(tenantID, applicationID, clientSecret string) (*GraphClient, error) {
	g := GraphClient{TenantID: tenantID, ApplicationID: applicationID, ClientSecret: clientSecret}
	g.apiCall.Lock()         // lock because we will refresh the token
	defer g.apiCall.Unlock() // unlock after token refresh
	return &g, g.gcAuthorize()
}
...
func (g *GraphClient) refreshToken() error {
	...
	where := g.TenantID // "common"
	resource := fmt.Sprintf("/%v/oauth2/v2.0/token", where)
	data := url.Values{}
...
// gcAuthorize currently not in use, but later it may become handy
func (g *GraphClient) gcAuthorize() error {
	if g.TenantID == "" {
		return fmt.Errorf("Tenant ID is empty")
	}
	resource := fmt.Sprintf("/%v/oauth2/authorize", g.TenantID)
	data := url.Values{}
	data.Add("grant_type", "client_credentials")
	data.Add("client_id", g.ApplicationID)
	data.Add("client_secret", g.ClientSecret)
	...
	q := req.URL.Query()
	q.Add("client_id", "7afd017d-0f44-4865-8b94-17de9a68832d")
	q.Add("scope", "https://graph.microsoft.com/mail.send") // openid profile offline_access https://graph.microsoft.com/user.read https://graph.microsoft.com/calendars.read
	req.URL.RawQuery = q.Encode()

	req.Header.Add("Content-Type", "application/x-www-form-urlencoded")
	// req.Header.Add("scope", "openid+profile+offline_access+user.read+calendars.read+mail.send")
	req.Header.Add("Content-Length", strconv.Itoa(len(data.Encode())))
...
// makeGETAPICall performs an API-Call to the msgraph API. This func uses sync.Mutex to synchronize all API-calls
func (g *GraphClient) makePOSTAPICall(apicall string, s interface{}, v interface{}) error {
	g.apiCall.Lock()
	defer g.apiCall.Unlock() // unlock when the func returns
	// Check token
	if g.token.WantsToBeRefreshed() { // Token not valid anymore?
	...
	// fmt.Println(req)
	return g.performPOSTRequest(req, v)
}

func (g *GraphClient) performPOSTRequest(req *http.Request, v interface{}) error {
	tr := &http.Transport{
		MaxIdleConns:       10,
	...
	// try for a maximum of 10 times to do the request
	for i := 0; i <= 10; i++ {
		// fmt.Println("graphclient's " + strconv.Itoa(i) + " attempt.")
		// if i > 0 {
		// 	// if not first attempt, wait 5 seconds
		// 	time.Sleep(2 * time.Second)
		// }
		resp, err := client.Do(req)
		if err != nil {
			errToReturn = fmt.Errorf("HTTP response error: %v of http.Request: %v", err, req.URL)
		}
		defer resp.Body.Close()                      // close body when func returns
		body, errReadAll = ioutil.ReadAll(resp.Body) // read body first to append it to the error (if any)
		if resp.StatusCode > 200 && resp.StatusCode < 299 && errReadAll == nil {
			// if the statuscode is ok - between 200 and 299 - and there is no error reading the response then break the loop
			// fmt.Println("response ok, loop breaks.")
			errToReturn = nil
			break
		} else {
			if resp.StatusCode < 200 || resp.StatusCode > 299 {
				...
				fmt.Println("Statuscode not ok:", resp.StatusCode)
				// json.Unmarshal(body, &v)
				errToReturn = fmt.Errorf("StatusCode is not OK: %v", resp.StatusCode) //json.NewEncoder(w).Encode(resp) // //, string(body))
			} else if errReadAll != nil {
				fmt.Println("couldn't read response")
				errToReturn = fmt.Errorf("HTTP response read error: %v of http.Request: %v", errReadAll, req.URL)
			}
		}
	...
	if errToReturn != nil {
		...
		return json.Unmarshal(body, &v) //json.NewEncoder(w).Encode(resp) // return the error of the json unmarshal
	}
...
// SendEmail is currently not in use
func (g *GraphClient) SendEmail(m *Mail) error {
	resource := "/messages/send" //"/sendMail"
	var resp string
	// m.SetGraphClient(g)
	err := g.makePOSTAPICall(resource, m, &resp)
	// fmt.Println(resp)
	return err
}
...
// ListDrives listing the available drives
func (g *GraphClient) ListDrives(uid string) (Drives, error) {
	// fmt.Println(uid)
	resource := fmt.Sprintf("/drives")
	var drive Drives
	err := g.makeGETAPICall(resource, nil, &drive)
	drive.SetGraphClient(g)
	return drive, err
}

// ListItems lists the available items within a drive
func (g *GraphClient) ListItems(driveID string) (Items, error) {
	resource := fmt.Sprintf("/drives/" + driveID + "/root/children")
	var items Items
	err := g.makeGETAPICall(resource, nil, &items)
	return items, err
}

// ListChildrenItems lists the children items of a given item
func (g *GraphClient) ListChildrenItems(driveID string, itemID string) (Items, error) {
	resource := fmt.Sprintf("/drives/" + driveID + "/items/" + itemID + "/children")
	var items Items
	err := g.makeGETAPICall(resource, nil, &items)
	return items, err
}

// CreateFolder creates a folder in the root directory
func (g *GraphClient) CreateFolder(driveID string, f *Folder) error {
	resource := fmt.Sprintf("/drives/" + driveID + "/root/children/")
	var marsh struct {
		resp string `json:"value"`
	}
	err := g.makePOSTAPICall(resource, f, &marsh)
	return err
}

// CreateProjectFolder creates a folder within a given item folder
func (g *GraphClient) CreateProjectFolder(driveID string, parentid string, f Folder) (Item, error) {
	resource := fmt.Sprintf("/drives/" + driveID + "/items/" + parentid + "/children/")
	var item Item
	err := g.makePOSTAPICall(resource, f, &item)
	return item, err
}

// CreateLinkToItem creates a shareable link to the given item
func (g *GraphClient) CreateLinkToItem(driveID string, itemID string, postType string) (LinkBase, error) {
	resource := fmt.Sprintf("/drives/" + driveID + "/items/" + itemID + "/createLink")
	var lb LinkBase
	type Post struct {
	...
	err := g.makePOSTAPICall(resource, p, &lb)
	return lb, err
}

func (g *GraphClient) GetDriveItemByPath(driveID string, itemPath string) (Item, error) {
	resource := fmt.Sprintf("/drives/" + driveID + "/root:/" + itemPath)
	var item Item
	err := g.makeGETAPICall(resource, nil, &item)
	return item, err
}
